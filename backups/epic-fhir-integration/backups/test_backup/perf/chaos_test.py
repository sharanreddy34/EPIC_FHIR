...
def test_network_timeout_retry(mock_responses, test_results):
    ...
    def fetch_with_retry():
        return client.get(f"Patient/{SANDBOX_PATIENT_ID}")
...
def test_server_error_recovery(mock_responses, test_results):
    ...
    # Add 500 response for first call
    mock_responses.add(
        responses.GET, 
        f"{base_url}/Patient/{SANDBOX_PATIENT_ID}",
        json={"resourceType": "OperationOutcome", "issue": [{"severity": "error", "code": "server-error"}]},
        status=500
    )
...
def test_real_api_resilience(real_client, test_results):
    ...
    for resource in resources:
        for i in range(3):  # Make multiple calls to same endpoints to test rate limiting
            try:
                if resource == "Patient":
                    endpoint = f"{resource}/{SANDBOX_PATIENT_ID}"
                else:
                    endpoint = f"{resource}?patient={SANDBOX_PATIENT_ID}"
                
                result = real_client.get(endpoint)
                assert result["resourceType"] in ["Bundle", resource]
                success_count += 1
